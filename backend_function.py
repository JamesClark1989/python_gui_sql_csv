...
def find_employee(search_term):
    connection = connect(database = 'employees.db')

    employee_db = connection.cursor()

    query = """SELECT emp_no, first_name, last_name, birth_date
FROM employees
WHERE (lower(first_name) LIKE '{}%')
OR (lower(last_name) LIKE '{}%')""".format(search_term,search_term)

    employee_db.execute(query)

    rows = employee_db.fetchall()
...
def find_employee_csv(search_term):
    with open('employees.csv', newline='') as csvfile:
        ...
        for employee in employee_data:
            if search_term in employee[2] or search_term in employee[3]:
